...
void ActivePowerControlCommand::setActivePowerLimit(const float limit, const PowerLimitControlType type)
{
    ...
    // For HMS 4CH inverters with firmware >= 1.1.12, use updated type values for persistent commands
    PowerLimitControlType actualType = type;
    if (_inv->supportsPowerDistributionLogic()) {
        if (type == PowerLimitControlType::AbsolutPersistent) {
            actualType = PowerLimitControlType::AbsolutPersistentHMS;
        } else if (type == PowerLimitControlType::RelativPersistent) {
            actualType = PowerLimitControlType::RelativPersistentHMS;
        }
    }

    // type
    _payload[14] = (actualType >> 8) & 0xff;
    _payload[15] = (actualType) & 0xff;
...
bool ActivePowerControlCommand::handleResponse(const fragment_t fragment[], const uint8_t max_fragment_id)
{
    ...
    if ((getType() == PowerLimitControlType::RelativNonPersistent) || (getType() == PowerLimitControlType::RelativPersistent) || (getType() == PowerLimitControlType::RelativPersistentHMS)) {
