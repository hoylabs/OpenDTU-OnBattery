...
void SunSpecInverterModel101_103::fill(uint16_t* ac, std::shared_ptr<InverterAbstract> const& inv, uint8_t unitId)
{
    ...
    uint32_t whTotal = (uint32_t)(yt * 1000.0f); // kWh -> Wh

    ac[0] = (uint16_t)(int16_t)(iac * 100.0f + 0.5f); // A: total AC current
    ...
    if (is3ph) {
        ...
        ac[1] = (uint16_t)(int16_t)(i1 * 100.0f + 0.5f); // AphA
        ac[2] = (uint16_t)(int16_t)(i2 * 100.0f + 0.5f); // AphB
        ac[3] = (uint16_t)(int16_t)(i3 * 100.0f + 0.5f); // AphC
    } else {
    ...
    if (is3ph) {
        ...
        ac[8] = (uint16_t)(int16_t)(v1 * 10.0f + 0.5f); // PhVphA
        ac[9] = (uint16_t)(int16_t)(v2 * 10.0f + 0.5f); // PhVphB
        ac[10] = (uint16_t)(int16_t)(v3 * 10.0f + 0.5f); // PhVphC
    } else {
        ac[8] = (uint16_t)(int16_t)(uac * 10.0f + 0.5f); // PhVphA: total for single-phase
        ac[9] = NI_U; // PhVphB: not applicable, single-phase
        ac[10] = NI_U; // PhVphC: not applicable, single-phase
    }

    ac[11] = (uint16_t)SF_V; // V_SF
    ac[12] = (uint16_t)(int16_t)(pac + 0.5f); // W: AC power
    ac[13] = (uint16_t)SF_W; // W_SF
    ac[14] = (uint16_t)(freq * 100.0f + 0.5f); // Hz
    ac[15] = (uint16_t)SF_HZ; // Hz_SF
    ac[16] = NI_S; // VA: apparent power - not computed
    ac[17] = (uint16_t)SF_W; // VA_SF
    ac[18] = NI_S; // VAr: reactive power - not computed
    ac[19] = (uint16_t)SF_W; // VAr_SF
    ac[20] = NI_S; // PF: power factor - not computed
    ac[21] = (uint16_t)SF_W; // PF_SF
    ac[22] = (uint16_t)(whTotal >> 16); // WH acc32 high word: lifetime energy
    ac[23] = (uint16_t)(whTotal & 0xFFFF); // WH acc32 low word
    ac[24] = (uint16_t)SF_WH; // WH_SF
    ac[25] = NI_U; // DCA: DC current - not measured on the AC side
    ac[26] = (uint16_t)SF_A; // DCA_SF
    ac[27] = NI_U; // DCV: DC voltage - not measured on the AC side
    ac[28] = (uint16_t)SF_V; // DCV_SF
    ac[29] = NI_S; // DCW: DC power - not measured on the AC side
    ac[30] = (uint16_t)SF_W; // DCW_SF

    bool hasTemp = stats->hasChannelFieldValue(TYPE_INV, CH0, FLD_T);
    ac[31] = hasTemp ? (uint16_t)(int16_t)(temp * 10.0f) : NI_S; // TmpCab
