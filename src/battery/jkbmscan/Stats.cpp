...
namespace Batteries::JkBmsCan
{
    ...
    void Stats::getLiveViewData(JsonVariant &root) const
    {
        ...
        addLiveViewValue(root, "Min_Cell_Temperature", _minCellTemperature, "°C", 1);
        addLiveViewValue(root, "Max_Cell_Temperature", _maxCellTemperature, "°C", 1);

        if (_hasMosfetTemperature)
        {
            addLiveViewValue(root, "Mosfet_Temperature", _mosfetTemperature, "°C", 1);
        }
    ...
    void Stats::mqttPublish() const
    {
        ...
        MqttSettings.publish("battery/minCellTemperature", String(_minCellTemperature));
        MqttSettings.publish("battery/maxCellTemperature", String(_maxCellTemperature));
        if (_hasMosfetTemperature)
        {
            MqttSettings.publish("battery/mosfetTemperature", String(_mosfetTemperature));
        }
        ...
        if (isVoltageValid() && isCurrentValid())
        {
            MqttSettings.publish("battery/power", String(getVoltage() * getChargeCurrent()));
        }
